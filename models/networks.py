...
class Discriminator(nn.Layer):
    def __init__(self, input_nc, ndf=64, n_layers=5):
        ...
        model = [
            nn.Pad2D([1, 1, 1, 1], 'reflect'),
            spectral_norm(nn.Conv2D(input_nc, ndf, kernel_size=4, stride=2, bias_attr=True)),
            nn.LeakyReLU(0.2)
        ]
        ...
        for i in range(1, n_layers-2):
            ...
            model += [
                nn.Pad2D([1, 1, 1, 1], 'reflect'),
                spectral_norm(nn.Conv2D(ndf*mult, ndf*mult*2, kernel_size=4, stride=2, bias_attr=True)),
                nn.LeakyReLU(0.2)
            ]
        ...
        model += [
            nn.Pad2D([1, 1, 1, 1], 'reflect'),
            spectral_norm(nn.Conv2D(ndf*mult, ndf*mult*2, kernel_size=4, stride=1, bias_attr=True)),
            nn.LeakyReLU(0.2)
        ]
        ...
        self.conv1x1 = nn.Conv2D(ndf*mult*2, ndf*mult, kernel_size=1, stride=1, bias_attr=True)
        self.leaky_relu = nn.LeakyReLU(0.2)

        self.pad = nn.Pad2D([1, 1, 1, 1], 'reflect')
        self.conv = spectral_norm(nn.Conv2D(ndf*mult, 1, kernel_size=4, stride=1, bias_attr=False))
